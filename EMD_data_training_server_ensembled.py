...
def separate_threads(folder,filepath,patient_data,problem_data,csv_folder,samplenumber,resume,initial_epoch):
	print ('Original IMF is training')
	...
	#For Original data
	csv_path = csv_folder+'Ensembled_train.csv'
	classes = C.disease_names
	X = []
	Y = []

	dataframe = pandas.read_csv(csv_path, header=None)
	dataset = dataframe.values
	X_prim = dataset[:,0:samplenumber].astype(float)
	Y_prim = dataset[:,samplenumber]

	'''
	#For PTB 7 class classification
	# for training Original/Original data
	...
	'''
	# for training Original/Original data
        indices = [s for s, x in enumerate(Y_prim) if x not in [ 'Noisy']]
        for f in indices:
                        X.append(X_prim[f])
                        Y.append(Y_prim[f])
	
	'''
	#For MIT-BIH Database training
	# for training Original/Ensembled data
	indices = [s for s, x in enumerate(Y_prim) if x not in ['AFL','B','T','VFL','NOD','VT','IVR']]
	for f in indices:
		X.append(X_prim[f])
		Y.append(Y_prim[f])
	...
	if resume=='False':
		...
		#load_architecture
		json_file = open(folder+'/Final_Weights/model.json','r')
		loaded_model_json = json_file.read()
		json_file.close()
		model = model_from_json(loaded_model_json)
		#Load weights
		weight_folder = folder+'/Saved_All_Weights/Saving_Each_Epoch'+'*'
	...
	#SAVE THE MODEL Architecture
		...
		mdl_save_path = folder+'/Final_Weights/model.json'
	...
	#Save Every epoch
	chk_path = folder+'/Saved_All_Weights/Saving_Each_Epoch.hdf5'
	...
	filehandler = open(folder+"/Training_History/History.obj","wb")
...
def Main():
	...
	deviceIDs=[]
	while not deviceIDs:
		deviceIDs = GPUtil.getAvailable(order='first',limit=1,maxMemory=0.10,maxLoad=0.99)
	...
	'''
	for i in range(resumed_IMF_number,number_of_IMFs+1):
	...
	separate_threads(folder,file_path,patient_data,problem_data,csv_folder,samplenumber,resume,initial_epoch)
	...
	#CM.Confusion_Matrix_Plot(Cnumber_of_IMFs)
