...
from keras.callbacks import TensorBoard,ModelCheckpoint,LearningRateScheduler,EarlyStopping,CSVLogger
...
def separate_threads(folder,IMF_number,filepath,patient_data,problem_data,csv_folder,samplenumber,resume,initial_epoch):
	...
	X_train,Y_train = Train_Data_Load(IMF_number,classes,total_training_data_length,samplenumber)
	...
	if C.grid_search:
		...
		history = model.fit(X_train,Y_train,validation_data=(X_validation,Y_validation),
						epochs=C.nb_epoch,batch_size=C.batch_size,verbose=1,shuffle=C.shuffle,callbacks =callback_list, initial_epoch=initial_epoch)
...
def Main():
	...
	class_4 = 4
	class_5 = 5
	for G in range(class_1,3):
		for H in range(class_2,4):
			for I in range(class_3,5):
				for J in range(class_4,6):
					for K in range(class_5,7):
						IMF_array = [G,H,I,J,K]
						print IMF_array
						folder = 'Outputs-all-combinations-'+str(G)+str(H)+str(I)+str(J)+str(K)
						FC.Folder_creation(number_of_IMFs,folder)
						separate_threads(folder,IMF_array,file_path,patient_data,problem_data,csv_folder,samplenumber,resume,initial_epoch)
					class_4 += 1
					class_5 = class_4 + 1
				class_3 += 1
				class_4 = class_3 + 1
		...
		class_5 = class_4 + 1
	'''
	folder = 'Outputs-combinations-'
	for i in range(0,len(C.IMF_array)):
		folder += str(C.IMF_array[i])
	FC.Folder_creation(number_of_IMFs,folder)
